//print stack trace, license at <enter license file>
function printStackTrace(a) {
    var a = a || {guess: !0}, b = a.e || null, a = !!a.guess, d = new printStackTrace.implementation, b = d.run(b);
    return a ? d.guessAnonymousFunctions(b) : b
}
...
printStackTrace.implementation.prototype = {run: function (a, b) {
...
}, instrumentFunction: function (a, b, d) {
    var a = a || window, c = a[b];
    a[b] = function () {
        d.call(this, printStackTrace().slice(4));
    ...
    for (var b =
        ...
        var e = b.exec(a[c]);
        e && d.push((e[3] || "{anonymous}") + "()@" + e[2] + ":" + e[1] + " -- " + a[c + 1].replace(/^\s+/, ""))
    }
    return d
}, opera9: function (a) {
    for (var b = /Line (\d+).*script (?:in )?(\S+)/i, a = a.message.split("\n"), d = [], c = 2, f = a.length; c < f; c += 2) {
        var e = b.exec(a[c]);
        e && d.push("{anonymous}()@" + e[2] + ":" + e[1] + " -- " + a[c + 1].replace(/^\s+/, ""))
    }
    return d
}, other: function (a) {
    for (var b = /function\s*([\w\-$]+)?\s*\(/i,
             d = [], c, f; a && a.arguments && 10 > d.length;)c = b.test(a.toString()) ? RegExp.$1 || "{anonymous}" : "{anonymous}", f = Array.prototype.slice.call(a.arguments || []), d[d.length] = c + "(" + this.stringifyArguments(f) + ")", a = a.caller;
    return d
}, stringifyArguments: function (a) {
    for (var b = [], d = Array.prototype.slice, c = 0; c < a.length; ++c) {
        var f = a[c];
        void 0 === f ? b[c] = "undefined" : null === f ? b[c] = "null" : f.constructor && (f.constructor === Array ? b[c] = 3 > f.length ? "[" + this.stringifyArguments(f) + "]" : "[" + this.stringifyArguments(d.call(f, 0, 1)) + "..." +
    ...
    for (var a, b = [function () {
        ...
        }], d = 0; d < b.length; d++)try {
        return a = b[d](), this.createXMLHTTPObject = b[d], a
    } catch (c) {
    }
    ...
    for (var b = 0; b < a.length; ++b) {
        ...
        if (f) {
            var e = d.exec(f[1]);
            e && (d = e[1], f = e[2], e = e[3] || 0, d && (this.isSameDomain(d) && f) && (d = this.guessAnonymousFunction(d, f, e), a[b] = c.replace("{anonymous}", d)))
        }
    }
    return a
    ...
    try {
        ...
        d = "getSource failed with url: " + a + ", exception: " + c.toString()
    }
    return d
}, findFunctionName: function (a, b) {
    for (var d = /function\s+([^(]*?)\s*\(([^)]*)\)/, c = /['"]?([0-9A-Za-z_]+)['"]?\s*[:=]\s*function\b/, f = /['"]?([0-9A-Za-z_]+)['"]?\s*[:=]\s*(?:eval|new Function)\b/,
             e = "", g, j = Math.min(b, 20), h, i = 0; i < j; ++i)if (g = a[b - i - 1], h = g.indexOf("//"), 0 <= h && (g = g.substr(0, h)), g)if (e = g + e, (g = c.exec(e)) && g[1] || (g = d.exec(e)) && g[1] || (g = f.exec(e)) && g[1])return g[1];
    return"(?)"
}};

//end of printstacktrace

//catch all errors and log them to database
window.onerror = function (message, url, linenumber) {
    postError(key, message, "caught", url, linenumber, "error");
    return throwError == 1;
};

//universal function to log custom errors or messages
function log(e, type, priority) {
    var stackMessage = printStackTrace({e: e});
    postError(key, stackMessage[0], type, stackMessage[1].substr(0, stackMessage[1].length - 5), stackMessage[1].substr(stackMessage[1].length - 5, 2), priority);
}
...
//Predefined callback function, to validate if errors are logged properly.
...
function postError(key, message, error_type, file_name, line, priority) {
    var exceptionLogUrl = baseUrl + "/logger/log_exception";
    requestServerCall(exceptionLogUrl + "?callback=jsonpCallback&project_key=" + key + "&file_name=" + file_name +
